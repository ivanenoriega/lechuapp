...
function obtenerProductos(req,res){
    ...
    conexion.query(sql, function (err, result) {
        if (err) return res.status(404).send("Hubo un error en la consulta producto");
        res.json(result);
    });
}
function obtenerProductosPorId(req, res) {
    ...
    const sql = "SELECT * FROM product P join category C ON P.id_category = C.id WHERE P.id = " + id;

    conexion.query(sql, function (err, result) {
        console.log(JSON.stringify(res.body));
        if (err) return console.log("Se ha producido un error en la consulta producto id", err);
        res.json(result);
