...
function listarOrdenes(req, res) {
  ...
  var sql =
    "select pu.id,c.nombre , p.nombre, p.precio, pu.cantidad, sum(p.precio * pu.cantidad) as total " +
  ...
  if (id) {
    ...
    conexion.query(sql, function (error, respuesta) {
      if (error) {
        return res
          .status(404)
          .send("Hubo un error en la consulta para traer ordenes por id");
      }
      if (respuesta == 0) {
        return res
          .status(404)
          .send("Hubo un error no hay una orden de compra con ese id");
      }
